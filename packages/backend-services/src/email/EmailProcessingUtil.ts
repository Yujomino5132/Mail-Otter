...
class EmailProcessingUtil {
  ...
  public static async generateGmailSummary(
    ...
    try {
      ...
      const result = await EmailProcessingUtil.generateSummaryCore(
        ...
        from, subject, extracted.text, message.threadId, isSummary, env, enabledApplicationIds, options,
      );
      if (!result) return null;
      return { message, ...result, application, accessToken, messageId, options };
    } catch (error: unknown) {
  ...
  public static async generateOutlookSummary(
    ...
    try {
      ...
      const result = await EmailProcessingUtil.generateSummaryCore(
        contextDAO, processedDAO, application, message.id,
        from, subject, body, message.conversationId || null, isSummary, env, enabledApplicationIds, options,
      );
      if (!result) return null;
      return { message, ...result, application, accessToken, messageId, options };
    } catch (error: unknown) {
  ...
  private static async generateSummaryCore(
    ...
    enabledApplicationIds: string[],
    options: EmailProcessingOptions,
  ): Promise<{ summaryHtml: string; actions: CreatedEmailAction[] } | null> {
    if (isSummary || EmailContentUtil.isFromMailbox(from, application.providerEmail)) {
      await processedDAO.markSkipped(application.applicationId, resolvedMessageId, 'Message was generated by the mailbox owner or Mail-Otter.');
      return null;
    }
    ...
    return { summaryHtml: EmailProcessingUtil.withActionSection(summary.html, actions), actions };
  }
